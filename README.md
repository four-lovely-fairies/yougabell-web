...
육아벨에서 사용자가 실제로 보는 화면 전부. 온보딩, 홈 대시보드, 미션, 마음 배터리, 챗봇, 주간 리포트가 여기 있다. 흥미로운 점은 이게 단독 웹사이트가 아니라는 것이다 — Expo 앱이 WebView로 띄우는 메인 UI다. 그래서 브라우저에서도 돌지만, 진짜 타깃은 네이티브 셸 안이다.
